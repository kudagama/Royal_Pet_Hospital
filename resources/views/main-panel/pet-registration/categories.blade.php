        ...
        <div class="w-full flex-grow flex flex-col bg-white rounded-lg shadow border">
            <div class="p-4 border-b bg-gray-50 flex justify-between items-center">
                <div class="relative w-full max-w-sm">
                    ...
                    <input type="text" id="searchCategory" onkeyup="renderCategories()" placeholder="Search categories..." class="w-full pl-10 pr-4 py-2 border rounded-lg text-sm bg-white focus:ring-blue-500 focus:border-blue-500">
                </div>
    ...
    <div class="bg-white rounded-lg w-full max-w-md p-6 shadow-2xl">
        ...
        <form id="categoryForm" onsubmit="saveCategory(event)">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="catName" required class="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Dog">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-bold text-gray-700 mb-1">Code / Prefix (Optional)</label>
                <input type="text" id="catCode" class="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. DG">
            </div>
...
<div id="breedsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center backdrop-blur-sm">
    <div class="bg-white rounded-lg w-full max-w-lg p-6 h-[80vh] flex flex-col shadow-2xl">
        ...
        <div class="flex gap-2 mb-4">
            <input type="text" id="newBreedName" class="flex-grow border rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Enter breed name...">
            <button onclick="addBreed()" class="bg-basic text-white px-4 py-2 rounded text-sm hover:bg-opacity-90 font-bold">Add</button>
        </div>
...
<script>
    ...
    function renderCategories() {
        ...
        if (filtered.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="px-6 py-8 text-center text-gray-400">No categories found.</td></tr>';
            return;
        }
        ...
        filtered.forEach(c => {
            ...
            tr.innerHTML = `
                    ...
                    <td class="px-6 py-4">
                        <span class="bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-0.5 rounded-full">${breedCount} Breeds</span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <button onclick="manageBreeds(${c.id})" class="text-blue-600 hover:text-blue-800 mr-4 font-bold text-xs uppercase tracking-wider">Manage Breeds</button>
                        <button onclick="editCategory(${c.id})" class="text-gray-600 hover:text-gray-800 mr-4 font-bold text-xs uppercase tracking-wider">Edit</button>
                        <button onclick="deleteCategory(${c.id})" class="text-red-500 hover:text-red-700 font-bold text-xs uppercase tracking-wider">Delete</button>
                    </td>
                 `;
    ...
    function saveCategory(e) {
        ...
        if (editingId) {
            ...
            categories.push({ id: Date.now(), name, code, breeds: [] });
        }
        saveData();
        closeModal();
    }
    ...
    function renderBreedsList() {
        ...
        cat.breeds.forEach((breed, index) => {
            ...
            li.innerHTML = `
                <span class="text-gray-800 font-bold text-sm">${breed}</span>
                <button onclick="deleteBreed(${index})" class="text-red-400 hover:text-red-600 transition-colors"><i class="bi bi-trash-fill"></i></button>
            `;
            list.appendChild(li);
        });
    }
